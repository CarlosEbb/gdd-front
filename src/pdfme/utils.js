...
export function agregarImageSandbox(templateJson, { jsonContent = null, width = null, height = null, base64Image = basetestvertical } = {}) {
  ...
  // Normalizar el base64 a data-URI. pdfme necesita el prefijo para detectar
  // el mime-type; sin él intenta decodificar como JPEG y lanza
  // "SOI not found in JPEG".
  const formattedImage = base64Image.startsWith('data:image') ? base64Image : `data:image/png;base64,${base64Image}`
  ...
  // Inyectar el valor en los inputs con el mismo data-URI ya normalizado,
  // para que pdfme no intente decodificarlo como JPEG.
  if (jsonContent && jsonContent.length > 0) {
    const primerObjeto = jsonContent[0]
    for (let i = 0; i < nuevoTemplate.schemas.length; i++) {
      primerObjeto[`TestTestLock${i + 1}`] = formattedImage
    }
  }
